...
if let myCastedButtonView = myView as? UIButton {
    myCastedButtonView.setTitle("Bye", for: .normal)
}
...
for type in library{
    if let movie = type as? Movie{
        print("Movie", movie.name, movie.director)
    }else if let song = type as? Song{
